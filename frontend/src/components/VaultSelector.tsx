...
export default function VaultSelector({ vaults, value, onChange }: Props) {
 ...
 <FolderOpen size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-fg-subtle pointer-events-none" />
 <select
 value={value}
 onChange={(e) => onChange(e.target.value)}
 className="pl-9 pr-8 py-2 text-sm border border-border rounded-lg bg-surface text-fg focus:outline-none focus:ring-2 focus:ring-accent appearance-none cursor-pointer"

...
export default function TodoModal({ todo, onClose, invalidateKeys, defaultAssigneeId }: Props) {
 ...
 {error && (
 <div className="bg-danger-bg border border-danger/30 text-danger dark:text-danger px-4 py-2 rounded-lg text-sm">
 {error}
 </div>
 )}
 ...
 <div className="border border-border rounded-lg px-3 py-2 max-h-36 overflow-y-auto space-y-1.5">
 {otherTodos.map((t) => (
 ...
 className="flex-1 bg-accent text-white py-2.5 rounded-lg font-semibold text-sm hover:bg-accent-hover disabled:opacity-50 transition-colors"
 >

...
export default function CommandPalette({ onClose, onNewTodo, onNewMeetingNote, onNewPersonalNote }: Props) {
 ...
 >
 <div
 className="bg-elevated border border-border rounded-xl shadow-overlay w-full max-w-2xl flex flex-col overflow-hidden"
 ...
 className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-sm bg-accent-1 text-accent-fg flex-shrink-0 hover:bg-accent-2"
 >
 {filter.label}
 ...
 className="flex-1 bg-transparent border-none outline-none text-fg placeholder:text-fg-faint text-sm"
 />
 <kbd className="hidden sm:inline-flex items-center text-2xs font-mono text-fg-muted px-1.5 h-5 rounded-sm border border-border bg-inset">
 esc
 </kbd>
 </div>
 ...
 {showingRecents && (
 <div className="px-4 pt-2 pb-1 text-2xs font-medium uppercase tracking-wider text-fg-subtle">
 Recent
 </div>
 )}
 {items.length === 0 ? (
 <div className="px-4 py-10 text-sm text-center text-fg-subtle">
 No results.
 </div>
 ) : (
 items.map((item, idx) => {
 ...
 <div key={item.id}>
 {isFirstNonRecent && (
 <div className="px-4 pt-3 pb-1 text-2xs font-medium uppercase tracking-wider text-fg-subtle">
 Actions
 </div>
 )}
 <button
 ...
 {item.type !== 'action' && (
 <span className="text-2xs uppercase tracking-wider text-fg-subtle flex-shrink-0">
 {TYPE_LABEL[item.type]}
 </span>
 )}
 {item.hint && (
 <span className="text-2xs font-medium text-fg-subtle flex-shrink-0 ml-1">
 {item.hint}
 </span>
 )}
 </button>
 ...
 <div className="flex items-center justify-between gap-4 px-4 py-2 border-t border-border text-xs text-fg-subtle">
